...
separate warm-up-and-drive events, and because an emissions fault is never
urgent enough to justify guessing. RIO borrows the concept and uses it
sparingly: exactly one monitor requires a drive cycle (the slow leak, where a
decline measured inside a single drive is mostly measuring the drive), and
nothing urgent waits for one. A critically low tire that sat through three
drives before being mentioned would be a design failure, not diagnostic rigour.
...
The speed heuristic exists only for the case nobody told us — the car moving
with no session open, which happens on a pod restart mid-drive and in every
headless test. It is a fallback, not a parallel truth.

WHAT A CYCLE RECORDS
--------------------
Enough to answer "was this fault present on more than one drive", and enough
that a freeze frame can name the drive it was captured on. Distance, speeds and
...
class DriveCycleTracker:
    """One tracker per process. Single-driver, like everything else here."""
    ...
    def observe(self, speed_mph: Optional[float], now: float,
        ...
        """Motion, once per engine tick. -> the cycle if one just began."""
        began = None
        moving = speed_mph is not None and speed_mph >= config.TIRE_DIAG_DRIVE_START_MPH
        ...
        if moving:
            ...
            if self._parked_since is None:
                self._parked_since = now
                  ...
                  and (now - self._parked_since) >= config.TIRE_DIAG_DRIVE_END_PARKED_S):
    ...
    def note_sensor(self, corner: str) -> None:
        if self.current is not None and corner not in self.current.sensors_seen:
            self.current.sensors_seen.append(corner)
    ...
    def _begin(self, by: str, now: float, session_id: Optional[str],
        ...
        cid = f"drive_{int(now)}_{self._seq:02d}"
        ...
        store.append_event("drive_cycle_start", {
    ...
    def end(self, why: str, now: float = None) -> Optional[dict]:
        ...
        store.append_event("drive_cycle_end", done, at=now)
        self.current = None
        self._parked_since = now
        return done
