...
# BGR overlay colours per state.
STATE_COLOR = {
    state_mod.COMFORTABLE: (90, 200, 90),
    state_mod.MONITOR: (120, 210, 200),
    state_mod.CLOSE: (60, 200, 255),
    state_mod.WARNING: (40, 130, 255),
    state_mod.URGENT: (50, 50, 255),
...
class GroundTruthDepth:
    ...
    def measure(self, frame, box, depth_map, frame_idx):
        ...
        if g is None or g.get("d_true") is None:
            return float("nan"), 0.0, {"reason": "no_gt"}
        d = float(g["d_true"])
...
def make_synthetic(path, fps=30.0, width=1280, height=720, v_host=25.0,
                   d0=60.0, hfov_deg=anchor_mod.HFOV_DEG,
                   cam_h=anchor_mod.CAMERA_HEIGHT_M):
    """Render a lead vehicle whose gap follows a scripted, physically consistent
    trajectory, and write per-frame ground truth alongside.

    Scenario is chosen to walk the entire state ladder and back down again:
      0-3 s    matched speed at 60 m            -> MONITOR
      3-6 s    lead brakes at 3 m/s²            -> CLOSE, then WARNING
      6-9 s    lead holds -9 m/s relative       -> URGENT
      9-12.4 s lead accelerates away at 5 m/s²  -> de-escalation (slow, 12 frames)
      12.4-16s gap opens at +8 m/s relative     -> back to COMFORTABLE
    ...
    for i in range(n):
        ...
        gt.append({
            ...
            "tau_true": round(d / max(v_host, 0.5), 4),
    ...
    with open(gt_path, "w") as fh:
        json.dump({"fps": fps, "width": width, "height": height,
                   "v_host": v_host, "hfov_deg": hfov_deg,
                   "camera_height_m": cam_h, "frames": gt}, fh, indent=1)
...
def draw_overlay(frame, rec, corridor, timeline, total_frames, show_corridor=True):
    ...
    cv2.putText(frame, st, (12, 34), cv2.FONT_HERSHEY_SIMPLEX, 1.0, color, 2, cv2.LINE_AA)
    if rec.get("new_lead"):
        cv2.putText(frame, "NEW_LEAD", (int(w * 0.44) - 130, 26),
                    cv2.FONT_HERSHEY_SIMPLEX, 0.55, (60, 200, 255), 2, cv2.LINE_AA)
...
def run(video_path, v_host, out_dir=None, depth_source="dav2", anchor_source="qwen",
    ...
    while True:
        ...
        trend = state_mod.classify_trend(d_dot, v_host, trend)
        ...
        confidence = state_mod.compute_confidence(
            depth_conf, quality, anchor_age, snap["coast_age"], ctx)
        ...
        if idx in gt_by_frame:
            rec["gt"] = {"d_true": gt_by_frame[idx]["d_true"],
                         "v_rel_true": gt_by_frame[idx]["v_rel_true"]}
    ...
    summary = _summarise(timeline, sm.transitions, idx, elapsed, fps, dt,
                         ...
                         depth_source, anchor_source, v_host)
...
def _summarise(timeline, transitions, n_frames, elapsed, fps, dt, innovations,
    ...
    runs = []
    for i, st in enumerate(timeline):
        if not runs or runs[-1]["state"] != st:
            runs.append({"state": st, "start_frame": i, "end_frame": i})
        else:
            runs[-1]["end_frame"] = i
    for r in runs:
    ...
    occupancy = {k: {"frames": v, "seconds": round(v * dt, 3),
                     "pct": round(100.0 * v / max(len(timeline), 1), 2)}
                 for k, v in sorted(occupancy.items(), key=lambda kv: -kv[1])}
    ...
    return {
        ...
        "state_timeline": runs,
        "transitions": [
            {"t": tr["t"], "from": tr["from"], "to": tr["to"], "reason": tr["reason"],
             "action": (tr["action"] or {}).get("kind") if tr["action"] else None}
...
def _print_summary(s):
    ...
    for r in s["state_timeline"]:
        print(f"  {r['start_s']:>7.2f}s -> {r['end_s']:>7.2f}s  "
              f"({r['duration_s']:>6.2f}s)  {r['state']}")
    print("-" * 72)
    print("transitions:")
    for tr in s["transitions"]:
        act = f"  [{tr['action']}]" if tr["action"] else ""
        print(f"  {tr['t']:>7.2f}s  {tr['from']:<12} -> {tr['to']:<12} "
              f"({tr['reason']}){act}")
    print("=" * 72)
...
def main():
    ...
    if args.make_synthetic:
        path, gt_path, n = make_synthetic(args.make_synthetic, fps=args.fps,
                                          v_host=args.v_host)
        print(f"wrote {path} ({n} frames @ {args.fps} fps)")
