...
**Status:** Phase A and Phase B shipped, verified against real frames from a
dashcam clip — acceptance tests 1–6, 54 checks. Every threshold and weight below
is provisional in the same sense as the headway ladder's: a starting point
chosen to be argued with against real drives, not a validated number. Everything
that feeds a decision is logged, so they can be.
...
| `CLARIFY_TTL_S` | 45.0 | how long an unanswered "which one?" stays answerable |
| `CLARIFY_MAX_CANDIDATES` | 3 | how many things a driver can hold while driving |
| `DECISIVE_MIN_MARGIN` | 0.15 | below this the question genuinely has two answers — see §12 |
...
## §12 Phase B — asking, forgetting, comparing, reading

### Ambiguity: ask, never guess

When nothing separates the candidates, RIO asks. Four conditions gate it, and
each rules out a way of asking wrongly: the reference must be genuinely
ambiguous, there must be more than one thing it could be, the question must be
*about an object* (a scene question has no referent to be unsure about, a
follow-up already knows what it is discussing), and RIO must not already be
waiting on an answer — asking twice in a row is worse than guessing once.

**The gate that made this work is in `resolve.py`, not here.** Phase A asked
Qwen to break a tie by showing it the frame with the candidates numbered. Given
a bare *"What is that?"* with a black saloon beside us and a white one a lane
over, Qwen picked a third vehicle on the far side of the road and reported
itself sure. It was not being stupid — the question contains no side, no class
and no colour, so nothing in the image distinguishes the right answer from the
wrong one. **The ambiguity is in the language, and looking harder at the road
does not resolve it.** So a reference with no discriminating cue never reaches
the model; it goes straight to ambiguous.

The second half is an absolute margin floor (`DECISIVE_MIN_MARGIN`). The
relative test alone passed on a difference of 0.07 between two cars that are
equally plausible to anyone looking out of the window. Measured on the test clip
at t=42.5 s: margin 0.000.

The question itself is generated by GPT-5.5, not templated. The candidate
descriptions are perception output — "car, right_adjacent_lane, 24 m, colour
black" — and the rule that no perception text is ever spoken to the driver does
not get an exception for being short. What it produces:

> *"The black sedan beside us, white sedan right, or car far left?"*

Nothing is committed as the referent while asking. That is the point: RIO does
not know which one yet, and recording a guess would make the answer irrelevant.

**The answer** is its own request type (`clarification_response` — the spec
names the type for asking, and the two-turn exchange needs one for answering
too). It is only ever produced when a question is outstanding, it resolves
*within the offered set only*, and it swaps the original question back in: the
driver said "the black one", but what they want to know is still "what is that".
Ordinals ("the second one") are positional references to the order RIO offered
them in, which no amount of looking at the road can resolve, so they are handled
separately.

### Lost objects

A referent outlives the object's presence in the buffer. When the vehicle has
gone, the stored crop is what the answer is based on, the *current* wide frame
goes too, and the grounding says plainly that the wide shot does not contain it
— without that, the model reconciles the two images by describing whatever car
is on that side of the road now. The rule it enforces: **never silently switch
vehicles.**

Verified by driving on until the track genuinely ages out of `membership.py`
(26 s of clip), then asking a follow-up. It answered about the car that had
gone, from the crop taken when it was last visible.

### Detail the image cannot support

Questions asking for a year, model, trim, engine or plate are flagged
(`router.wants_fine_detail`) and the grounding tells the model to give it only
if the image carries it, and otherwise to say what the shape *does* narrow it
down to. `CROP_DETAIL_LIMIT_PX` was raised from 96 to 160 after looking at what
the numbers correspond to: the saloon GPT-5.5 identifies correctly sits at
245 px, a car 70 m back comes in around 120 px and carries no readable badge at
all, and 96 was letting the second case through unflagged.

Note the interaction with frame selection: because the selector searches ~6 s
for the clearest view, a vehicle that is 16 px in the current frame is often
120–220 px in the frame actually chosen. The system is good at *not* being
detail-limited, which is why the flag alone is not relied on and the guidance
goes with every fine-detail question.

### Comparisons

Two references are split out of the question (`"which is bigger, the truck or
the white car?"`), resolved independently, and cropped **from the same frame** —
a comparison of two different moments is not a comparison. Two references that
resolve to the same object mean one was matched on weaker evidence, so the
second falls back to its next-best candidate. When the driver names only one
thing (*"which one is faster?"* names nothing) it degrades to a plain object
question rather than inventing the other half.

### Reading text

**Nothing in the detector's vocabulary is a sign.** RF-DETR gives us
person/bicycle/car/motorcycle/bus/truck, so a "what does that say" reference
never matches a tracked object. Consequences, both deliberate:

- The wide frame goes at **high** detail (`READ_TEXT_FRAME_DETAIL`) and there is
  usually no crop. Resolution is what reading needs.
- Clarification is **disabled** for this type. Since resolution is always
  ambiguous here, the generic path would ask "the white saloon, or the black
  one?" about a road sign. Observed exactly once, which was enough.
- A text question that scores weakly onto a vehicle drops the referent rather
  than cropping it — a close-up of a boot lid invites the model to read words
  off it.

The honest result: *"I can see a small roadside sign up ahead, but it's too far
and blurry to read. Just a little white rectangle in the distance."*

### Still not built

`location_or_landmark_question` is classified and marked `phase_b`, then handled
as the nearest available behaviour. Doing it properly means joining the scene to
the nav route's actual position, which is client-side (`rio_navcore.js`) and not
something the server currently knows — see `nav.latest_route()`'s docstring.
...
```
python -m tools.visual_selftest                  # acceptance 1-6, real frames
python -m tools.visual_selftest --phase A        # tests 1-3 only
python -m tools.visual_selftest --phase B        # tests 4-6 only
...
Test 5 needs a vehicle that actually leaves the view and test 4 needs a scene
with two equally plausible ones, so Phase B runs test 5 first and drives on to
`--ambiguous-at` (frame 160 of the reference clip) for test 4. Both conditions
are properties of the footage, not simulated.

**One operational note that costs an afternoon if you do not know it:**
`pkill -f "uvicorn app:app"` also matches the shell running the pkill, so it
kills itself and leaves the old server alive. Two servers then load Qwen onto
the same GPU and the second dies with "Cannot copy out of meta tensor", which
reads like a model bug and is not one. Use `pgrep -f "uvicorn app:[a]pp" |
xargs -r kill -9`, and check `nvidia-smi --query-compute-apps` before believing
anything subtler.
